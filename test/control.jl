...
function brachistochrone_attitude()
    ...
    n = 600
    # tstep = Tf / n
    @variable(model, Tf, start = 60.0)
    @constraint(model, Tf >= 0.1)
    ...
    # dynamic constraints
    ddt_w(x::Matrix, k::Int) = (x[:, k] .- x[:, k-1]) ./ (Tf / n)
    ddt_C(X::Array, w::Array, k::Int) = (X[:, :, k] .- X[:, :, k-1]) ./ (Tf / n)
    ...
    @objective(model, Min, Tf + 0.01 * sum(u[1, :] .^ 2 + u[2, :] .^ 2 + u[3, :] .^ 2))
    ...
    println("inner angle: ", ang)
    println("minimum time: ", value(Tf))
